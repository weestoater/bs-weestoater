...
-- View: All matches with goals and cards aggregated
...
GROUP BY m.id, m.season_id, m.match_date, m.opposition, m.venue, 
         m.goals_scored, m.goals_conceded, m.league, m.video_url, 
         m.iplayer_url, m.notes, m.created_at, m.updated_at;
